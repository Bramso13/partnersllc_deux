...
export async function POST(
  ...
  try {
    ...
    // Update step instance to REJECTED
    ...
    if (updateError) {
      ...
      return NextResponse.json(
        { error: "Erreur lors du rejet de l'étape" },
        { status: 500 }
      );
    }

    // Optionally reject all PENDING fields in this step
    // This helps maintain consistency
    await supabase
      .from("step_field_values")
      .update({
        validation_status: "REJECTED",
        rejection_reason: "Étape rejetée par l'administrateur",
        reviewed_by: agentId,
    ...
    console.error("Error in reject step endpoint:", error);
    return NextResponse.json(
