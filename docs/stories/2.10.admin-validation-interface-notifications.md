...
2. Two tabs: "Champs à valider" (Field Values) and "Étapes à valider" (Step Instances)
3. "Champs à valider" tab displays table with columns: Client, Dossier, Étape, Champ, Valeur, Date, Statut, Actions
4. "Étapes à valider" tab displays table with columns: Client, Dossier, Étape, Champs validés, Date soumission, Statut, Actions
5. Filter dropdown: Tous, En attente, En révision, Validés, Refusés
...
15. Character counter below textarea: "X/500 caractères"
16. Validation error if rejection reason too short
17. Success toast after approval: "Champ validé avec succès"
18. Success toast after rejection: "Champ refusé. Email envoyé au client."
...
24. Visual progress: "X/Y champs validés" with progress bar
...
27. Step-level "Valider l'étape" button enabled only when all fields APPROVED
28. Step-level "Rejeter l'étape" button with overall rejection textarea
...
44. Subject: "PARTNERS LLC - Étape validée : [Step Name]"
...
47. Progress indicator: "Étape X/Y complétée"
...
```
┌────────────────────────────────────────────┐
│ Header: Validation des dossiers     [Badge]│
├────────────────────────────────────────────┤
│ [Champs à valider] [Étapes à valider]      │
│                                            │
│ [Filter ▾] [Search: ________] [25/page ▾] │
├────────────────────────────────────────────┤
│ TABLE                                      │
│ ┌────┬──────┬──────┬──────┬──────┬─────┐ │
│ │ Cl │ Doss │ Étap │ Chmp │ Stat │ Act │ │
│ ├────┼──────┼──────┼──────┼──────┼─────┤ │
│ │... │ ...  │ ...  │ ...  │ ...  │ ... │ │
│ └────┴──────┴──────┴──────┴──────┴─────┘ │
│                                            │
...
**Field Value Status:**
- PENDING: `bg-brand-warning/10 text-brand-warning border-brand-warning` + ⏳ icon
- APPROVED: `bg-brand-success/10 text-brand-success border-brand-success` + ✓ icon
- REJECTED: `bg-brand-danger/10 text-brand-danger border-brand-danger` + ✗ icon

**Step Instance Status:**
- DRAFT: `bg-gray-500/10 text-gray-500 border-gray-500` + ✏️ icon
- SUBMITTED: `bg-blue-500/10 text-blue-500 border-blue-500` + 📨 icon
- UNDER_REVIEW: `bg-purple-500/10 text-purple-500 border-purple-500` + 👁️ icon
- APPROVED: `bg-brand-success/10 text-brand-success border-brand-success` + ✓ icon
- REJECTED: `bg-brand-danger/10 text-brand-danger border-brand-danger` + ✗ icon
...
│ • Pattern: ^[^@]+@[^@]+\.[^@]+$ │
│                                  │
│ Soumis le: 07/01/2026 14:30     │
│                                  │
│ ────────────────────────────────│
│ Historique des révisions:        │
│ • 05/01 14:00 - Refusé par Alice│
│   "Format invalide"             │
│ • 07/01 14:30 - Resoumis        │
│                                  │
│ ────────────────────────────────│
│ [Approuver] [Rejeter]           │
└──────────────────────────────────┘
...
<Html>
  ...
  <Body style={{ backgroundColor: '#F5F5F5', fontFamily: 'Inter, sans-serif' }}>
    <Container style={{ maxWidth: '600px', margin: '40px auto', backgroundColor: '#FFFFFF', borderRadius: '16px', overflow: 'hidden' }}>
      ...
      <div style={{ padding: '0 32px 32px' }}>
        <Heading style={{ color: '#191A1D', fontSize: '24px', marginBottom: '16px' }}>
          Correction requise
        </Heading>

        <Text style={{ color: '#666666', fontSize: '16px', lineHeight: '24px' }}>
          Bonjour {clientName},
        </Text>

        <Text style={{ color: '#666666', fontSize: '16px', lineHeight: '24px' }}>
          Nous avons examiné votre soumission et un champ nécessite une correction :
        </Text>

        {/* Field Info Card */}
        <div style={{ backgroundColor: '#F9F9F9', padding: '20px', borderRadius: '12px', margin: '20px 0', borderLeft: '4px solid #F95757' }}>
          <Text style={{ color: '#191A1D', fontWeight: 'bold', margin: '0 0 8px' }}>
            📋 {fieldLabel}
          </Text>
      ...
      {/* Footer */}
      <div style={{ backgroundColor: '#F9F9F9', padding: '24px 32px', borderTop: '1px solid #E5E5E5' }}>
        <Text style={{ color: '#999999', fontSize: '12px', textAlign: 'center', margin: '0' }}>
          © 2026 PARTNERS LLC - Tous droits réservés
        </Text>
        <Text style={{ color: '#999999', fontSize: '12px', textAlign: 'center', margin: '8px 0 0' }}>
          <a href="mailto:support@partnersllc.com" style={{ color: '#00F0FF', textDecoration: 'none' }}>
            Besoin d'aide ?
          </a>
        </Text>
      </div>
...
export async function POST(request: Request, { params }: { params: { id: string } }) {
  ...
  if (!rejection_reason || rejection_reason.length < 10) {
    return new Response('Rejection reason must be at least 10 characters', { status: 400 });
  }
...
export async function sendStepRejectionEmail(stepInstance: any, rejectedFields: any[]) {
